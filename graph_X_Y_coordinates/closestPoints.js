...
// Manhattan distance & MinHeap
// Time: O(n * log(n))...n points...insertion/extraction of heap is log(n)
// Space: O(n)
var kClosest = function(points, k) {
    const minHeap = new MinHeap(), result = []
    
    for (let [x, y] of points) {
        let val = Math.abs(x)**2 + Math.abs(y)**2
        minHeap.insert(new Node([x, y], val))
    }
...
console.log(kClosest(points = [[3,3],[5,-1],[-2,4]], k = 2)) // = [[3,3],[-2,4]]