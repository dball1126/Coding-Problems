class SegmentTree {
    ...
    build(idx, start, end) {
        if (start === end) {
            return this.tree[idx] = this.arr[idx];
        }
        let mid = Math.floor((start + end) / 2)
        this.build(2 * idx + 1, start, mid)
        this.build(2 * idx + 2, mid + 1, end)
        return this.tree[2 * idx + 1] + this.tree[2 * idx + 2]
    ...
    _query(nde, start, end, left, right) {
        if (right < start || left > end) { // is start end out side of left and right range
            return 0;
        }
        if (left <= start && right <= end) { // is the start end range completely inside of the left and right range
